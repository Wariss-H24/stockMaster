  ...
  <div v-else class="layout" :class="{ 'sidebar-open': sidebarOuverte }">
    ...
    <aside class="sidebar">
      ...
      <nav class="sidebar-nav">
        ...
        <router-link to="/entrepots" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M3 9.5L12 4l9 5.5V20H3V9.5z" stroke="currentColor" stroke-width="1.8"/><path d="M9 21V12h6v9" stroke="currentColor" stroke-width="1.8"/></svg>
          <span>Entrepôts</span>
        </router-link>
        <router-link to="/zones" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><rect x="3" y="3" width="8" height="10" rx="1.5" stroke="currentColor" stroke-width="1.8"/><rect x="13" y="3" width="8" height="6" rx="1.5" stroke="currentColor" stroke-width="1.8"/><rect x="13" y="13" width="8" height="8" rx="1.5" stroke="currentColor" stroke-width="1.8"/><rect x="3" y="17" width="8" height="4" rx="1.5" stroke="currentColor" stroke-width="1.8"/></svg>
          <span>Zones</span>
        </router-link>
        <router-link to="/produits" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M20 7H4a1 1 0 00-1 1v10a1 1 0 001 1h16a1 1 0 001-1V8a1 1 0 00-1-1z" stroke="currentColor" stroke-width="1.8"/><path d="M16 7V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2" stroke="currentColor" stroke-width="1.8"/></svg>
          <span>Produits</span>
        </router-link>
        <router-link v-if="auth.aUnRole('ADMIN','GESTIONNAIRE')" to="/categories" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M4 6h16M4 12h16M4 18h16" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>
          <span>Catégories</span>
        </router-link>
        <router-link to="/fournisseurs" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M12 4a4 4 0 110 8 4 4 0 010-8z" stroke="currentColor" stroke-width="1.8"/><path d="M6 20v-2a4 4 0 014-4h4a4 4 0 014 4v2" stroke="currentColor" stroke-width="1.8"/></svg>
          <span>Fournisseurs</span>
        </router-link>
        <router-link to="/stocks" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M4 7h16M4 12h16M4 17h16" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>
          <span>Stocks</span>
        </router-link>
        <router-link to="/mouvements-stock" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M5 12h14M12 5l7 7-7 7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
          <span>Mouvements</span>
        </router-link>
        <router-link to="/bon-receptions" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M4 6h16M4 10h16M4 14h8" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>
          <span>Bons réception</span>
        </router-link>
        <router-link to="/bon-sorties" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M4 6h16M4 12h16M4 18h8" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><path d="M19 8v8" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>
        ...
        <router-link v-if="auth.aUnRole('ADMIN','GESTIONNAIRE','MAGASINIER')" to="/transferts" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M4 12h7" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><path d="M13 6l5 6-5 6" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/><path d="M21 7v10" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>
        ...
        <router-link v-if="auth.aUnRole('ADMIN','GESTIONNAIRE')" to="/inventaires" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2" stroke="currentColor" stroke-width="1.8"/><rect x="9" y="3" width="6" height="4" rx="1" stroke="currentColor" stroke-width="1.8"/><path d="M9 12h6M9 16h4" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>
          <span>Inventaires</span>
        </router-link>
        <router-link to="/alertes" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9" stroke="currentColor" stroke-width="1.8"/><path d="M13.73 21a2 2 0 01-3.46 0" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/></svg>
          <span>Alertes</span>
        ...
        <router-link v-if="auth.aUnRole('ADMIN','GESTIONNAIRE','AUDITEUR')" to="/reporting" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" stroke="currentColor" stroke-width="1.8"/><polyline points="14 2 14 8 20 8" stroke="currentColor" stroke-width="1.8"/></svg>
          <span>Reporting</span>
        </router-link>
        <router-link v-if="auth.aUnRole('ADMIN','GESTIONNAIRE')" to="/commandes-fournisseurs" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z" stroke="currentColor" stroke-width="1.8"/><line x1="3" y1="6" x2="21" y2="6" stroke="currentColor" stroke-width="1.8"/><path d="M16 10a4 4 0 01-8 0" stroke="currentColor" stroke-width="1.8"/></svg>
          <span>Commandes</span>
        </router-link>
        <router-link v-if="auth.aUnRole('ADMIN','AUDITEUR')" to="/tracabilite" class="nav-item" @click="sidebarOuverte = false">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" stroke="currentColor" stroke-width="1.8"/><path d="M9 12l2 2 4-4" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>
    ...
    <div class="main-wrapper">
      <header class="topbar">
        ...
        <div class="topbar-right"><span class="topbar-badge">v1.0</span></div>
      </header>
...
export default {
  ...
  beforeUnmount() { clearInterval(this._alerteTimer) },
  computed: {
    titreRoute() {
      const titres = {
        '/tableau-de-bord': 'Tableau de bord', '/entrepots': 'Entrepôts',
        '/zones': 'Zones', '/produits': 'Produits', '/categories': 'Catégories',
        '/fournisseurs': 'Fournisseurs', '/stocks': 'Stocks',
        '/mouvements-stock': 'Mouvements', '/bon-receptions': 'Bons de réception',
        '/bon-sorties': 'Bons de sortie', '/transferts': 'Transferts',
        '/inventaires': 'Inventaires', '/alertes': 'Alertes stock',
        '/reporting': 'Reporting & Exports', '/tracabilite': 'Traçabilité & Audit',
        '/commandes-fournisseurs': 'Commandes fournisseurs',
        '/emplacements': 'Emplacements', '/qrcode': 'QR Code & Scanner',
  ...
  methods: {
    async chargerAlertes() {
      ...
      try { const res = await alerteApi.critiques(); this.nbAlertes = res.data.length } catch { /* */ }
    },
    deconnecter() { authStore.logout(); this.$router.push('/login') }
  }
}
</script>
...
:root {
  --navy: #1e3a5f; --navy-light: #2a4f80; --navy-xlight: #e8eef6;
...
html { font-size: 15px; }
body { font-family: 'Inter','Segoe UI',system-ui,sans-serif; background: var(--gray-100); color: var(--gray-900); line-height: 1.5; }
...
.sidebar-close { display: none; background: none; border: none; color: rgba(255,255,255,.6); cursor: pointer; padding: 4px; border-radius: var(--radius-sm); }
.sidebar-close:hover { color: #fff; background: rgba(255,255,255,.1); }

/* NAV — pas de scroll, items compacts */
.sidebar-nav {
...
.nav-item.router-link-active { background: rgba(37,99,235,.4); color: #fff; font-weight: 600; }
...
.badge-notif {
  ...
  border-radius: 999px; line-height: 1.6; min-width: 16px; text-align: center;
}
...
.logout-btn {
  ...
  color: rgba(255,255,255,.4); padding: 6px; border-radius: var(--radius-sm);
  display: flex; align-items: center; flex-shrink: 0; transition: color .15s, background .15s;
}
...
.burger-btn { display: none; background: none; border: none; color: var(--gray-700); cursor: pointer; padding: 6px; border-radius: var(--radius-sm); transition: background .15s; }
.burger-btn:hover { background: var(--gray-100); }
.topbar-title { font-size: .95rem; font-weight: 600; color: var(--gray-900); flex: 1; }
.topbar-badge { font-size: .72rem; font-weight: 600; color: var(--gray-500); background: var(--gray-100); border: 1px solid var(--gray-200); padding: 3px 8px; border-radius: 999px; }
...
/* CARDS */
.card { background: var(--white); border-radius: var(--radius); border: 1px solid var(--gray-200); box-shadow: var(--shadow); padding: 24px; }

/* STATS GRID */
.stats-grid { display: grid; grid-template-columns: repeat(4,1fr); gap: 16px; margin-bottom: 24px; }
.stat-card { background: var(--white); border: 1px solid var(--gray-200); border-radius: var(--radius); box-shadow: var(--shadow); padding: 20px 22px; display: flex; align-items: flex-start; gap: 14px; }
...
.stat-label { font-size: .75rem; font-weight: 600; text-transform: uppercase; letter-spacing: .06em; color: var(--gray-500); }
.stat-value { font-size: 1.75rem; font-weight: 700; color: var(--gray-900); line-height: 1.2; margin-top: 3px; }

/* TABLE */
.table-wrap { overflow-x: auto; }
table { width: 100%; border-collapse: collapse; font-size: .875rem; }
thead th { background: var(--gray-50); color: var(--gray-500); font-weight: 600; font-size: .72rem; text-transform: uppercase; letter-spacing: .07em; padding: 10px 16px; text-align: left; border-bottom: 1px solid var(--gray-200); white-space: nowrap; }
...
/* BADGES */
.badge { display: inline-flex; align-items: center; padding: 3px 10px; border-radius: 999px; font-size: .72rem; font-weight: 600; letter-spacing: .02em; white-space: nowrap; }
...
/* BOUTONS */
.btn { display: inline-flex; align-items: center; gap: 6px; padding: 8px 16px; border-radius: var(--radius-sm); border: 1px solid transparent; cursor: pointer; font-size: .85rem; font-weight: 600; line-height: 1; transition: all .15s; white-space: nowrap; }
...
.form-input, .form-select { width: 100%; padding: 8px 12px; border: 1px solid var(--gray-300); border-radius: var(--radius-sm); font-size: .875rem; color: var(--gray-900); background: var(--white); transition: border-color .15s, box-shadow .15s; outline: none; }
...
/* MODAL */
.modal-overlay { position: fixed; inset: 0; background: rgba(15,23,42,.5); display: flex; align-items: center; justify-content: center; z-index: 400; padding: 16px; }
.modal { background: var(--white); border-radius: var(--radius-lg); width: 100%; max-width: 500px; max-height: 90vh; overflow-y: auto; box-shadow: var(--shadow-lg); animation: modal-in .18s ease; }
@keyframes modal-in { from { opacity: 0; transform: translateY(-12px) scale(.98); } to { opacity: 1; transform: none; } }
.modal-header { display: flex; align-items: center; justify-content: space-between; padding: 20px 24px 0; }
.modal-title { font-size: 1.05rem; font-weight: 700; color: var(--gray-900); }
.modal-close { background: none; border: none; cursor: pointer; color: var(--gray-400); padding: 4px; border-radius: var(--radius-sm); display: flex; align-items: center; justify-content: center; }
.modal-close:hover { background: var(--gray-100); color: var(--gray-700); }
.modal-body { padding: 20px 24px; }
.modal-footer { display: flex; gap: 10px; justify-content: flex-end; padding: 16px 24px 20px; border-top: 1px solid var(--gray-100); }

/* PROGRESS BAR */
.progress-bar { background: var(--gray-200); border-radius: 999px; height: 5px; overflow: hidden; flex: 1; }
...
.search-box { display: flex; align-items: center; gap: 8px; border: 1px solid var(--gray-300); border-radius: var(--radius-sm); padding: 7px 12px; background: var(--white); transition: border-color .15s, box-shadow .15s; }
...
.search-box input { border: none; outline: none; font-size: .875rem; color: var(--gray-900); background: transparent; width: 200px; }
.search-box input::placeholder { color: var(--gray-400); }

/* FORM ERROR */
.form-error { font-size: .8rem; color: var(--danger); background: var(--danger-bg); border: 1px solid #fca5a5; border-radius: var(--radius-sm); padding: 8px 12px; margin-top: 4px; }
...
@media (max-width: 480px) { .stats-grid { grid-template-columns: 1fr 1fr; } .topbar { padding: 0 14px; } }
</style>
